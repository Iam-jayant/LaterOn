...
export const registerAdminRoutes = (app: FastifyInstance): void => {
  app.post("/api/admin/risk-keeper/run", async (request, reply) => {
    ...
    try {
      ...
      const plans = await app.ctx.repository.getActivePlans();
      
      app.log.info(`Risk keeper processing ${plans.length} active/late plans`);
      ...
      for (const plan of plans) {
        ...
        try {
          ...
          if (shouldSettle && previousStatus !== expectedStatus) {
            ...
            const updatedPlan = await app.ctx.gateway.settleRisk(plan.planId, currentUnix);
          ...
          app.log.error({
            planId: plan.planId,
            error: errorMessage,
          }, "Failed to settle risk for plan");
      ...
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      app.log.error({ error: errorMessage }, "Risk keeper execution failed");
  ...
  app.get("/api/health", async (request, reply) => {
    try {
      ...
      await app.ctx.repository.healthCheck();
